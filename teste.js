var raio = 0.125, veloMaxima = 2.3, veloMaxima = 2.3, tempfinal = 0;
...
function vetor() {
    ...
    x = document.getElementById("x"); //pega a posicao inicial de x
    y = document.getElementById("y"); //pega a posicao inicial de y
    vetRoboX[0] = parseFloat(x.value);
    vetRoboY[0] = parseFloat(y.value);
    alert("X: " + vetRoboX[0]);
    alert("Y: " + vetRoboY[0]);
    var distBolaRobo = document.getElementById("resultado");
    vetDistancia[0] = Math.sqrt(Math.pow((vetX[0] - vetRoboX[0]), 2) + Math.pow((vetY[0] - vetRoboY[0]), 2));
    //var dist = parseFloat(vetDistancia.toFixed(3));

    var i = 0, n = 0, z = 0, a = 0, b = 0, c = 0;

    for (b=0; b<1002; b++){ //coloca os valores da aceleracao no vetor
        aceleracaoBolaX[b] = -0.01;
        aceleracaoBolaY[b] = -0.016;
        aceleracao[b] = 2.6;

    }


    for (a = 0; a<1002; a++){ //calcula a velocidade da bola em X
        veloBolaX[a] = (-0.01*vetTemp[a]) + 0.5;
    }


    for (z = 0; z<1002; z++){ //calcula a velocidade da bola em Y
        veloBolaY[z] = (-0.016*vetTemp[z]) + 0.4;
    }

    for (n = 0; n < 1002; n++) { //calcula a aceleracao do robo

        veloRobo[n] = aceleracao[n] * vetTemp[n];

        if (veloRobo[n] >= veloMaxima) { //limita a velocidade do robo em veloMaxima
            veloRobo[n] = veloMaxima;
            aceleracao[n] = 0; //se a velocidade for maxima(constante), aceleracao é 0

        }

    }
    if (veloRobo >= veloMaxima){
        aceleracao = 0;
    }

    for (i = 0; i < 1002; i++) {

        vetDistancia[i] = parseFloat(Math.sqrt((Math.pow((vetX[i] - vetRoboX[i]), 2) + Math.pow((vetY[i] - vetRoboY[i]), 2))));

        cos[i] = parseFloat((vetX[i] - vetRoboX[i]) / vetDistancia[i]); //acha o cosseno
        sen[i] = parseFloat((vetY[i] - vetRoboY[i]) / vetDistancia[i]); //acha o seno


        vetRoboX[i + 1] = parseFloat(vetRoboX[i] + (cos[i] * 0.04)); //vai aumentando a posicao do robo em X
        vetRoboY[i + 1] = parseFloat(vetRoboY[i] + (sen[i] * 0.05)); //vai aumentando a posicao do robo em Y


        if (vetRoboX[i] >= vetX[i] - 0.125 && vetRoboX[i] <= vetX[i] + 0.125 && vetRoboY[i] >= vetY[i] - 0.125 && vetRoboY[i] <= vetY[i] + 0.125) { //hitbox - range de 0.125
            tempfinal = vetTemp[i];

            for (c=0;c<1002;c++){
                tempFinal[c] = vetTemp[c];
                if(tempFinal[c] == tempfinal){
                    break;
                }
            }
            alert("Tempo: " + tempFinal);
            alert("Distancia 1: " + vetDistancia[0] + "distancia final: " + vetDistancia[i]);
            break;
        }
...
function adicionaGrafico(){ //plota todos os graficos
    var trace1 = {
        x: vetRoboX,
        y: vetRoboY,
        mode: 'lines+markers',
...
//====================X/Y por tempo==============================================
    ...
    var layout = {
        title: 'X do Robo e Bola pelo Tempo'
    };
...
//======================
    ...
    //=================VX bola e robo/ T=========================================
    var veloRobox = {
        x: tempFinal, //SE NAO FOR ASSIM, TROCAR PARA vetTemp ==========
        y: veloRobo,
        mode: 'lines+markers',
        name: 'Velocidade do Robo no eixo X pelo Tempo',
        line: {
            color: 'rgb(0, 0, 0)',
            width: 1
        }
    ...
    var veloBolax = {
        ...
        y: veloBolaX,
        mode: 'lines+markers',
        name: 'Velocidade da Bola no eixo X pelo tempo',
        line: {
            color: 'rgb(255, 69, 0)',
            width: 1
        }
    ...
    var veloRoboy = {
        ...
        y: veloRobo,
        mode: 'lines+markers',
        name: 'Velocidade do Robo no eixo Y pelo Tempo',
        line: {
            color: 'rgb(0, 0, 0)',
            width: 1
        }
    ...
    var veloRoboY = {
        ...
        y: veloBolaY,
        mode: 'lines+markers',
        name: 'Velocidade da Bola no eixo Y pelo tempo',
        line: {
            color: 'rgb(255, 69, 0)',
            width: 1
        }
    ...
    var aRobox = {
        ...
        y: aceleracao,
        mode: 'lines+markers',
        name: 'Aceleração do Robo no eixo X pelo Tempo',
        line: {
            color: 'rgb(0, 0, 0)',
            width: 1
        }
    ...
    var aBolax = {
        ...
        y: aceleracaoBolaX,
        mode: 'lines+markers',
        name: 'Aceleração da Bola no eixo X pelo tempo',
        line: {
            color: 'rgb(255, 69, 0)',
            width: 1
        }
    ...
    var aBolay = {
        ...
        y: aceleracaoBolaY,
        mode: 'lines+markers',
        name: 'Aceleração da Bola no eixo Y pelo tempo',
        line: {
            color: 'rgb(128,128,128)',
            width: 1
        }
    };
    var data = [aRobox, aBolax, aBolay];
    var layout = {
        title: 'X e Y do robo e bola pelo tempo'
    };
    Plotly.newPlot('myDiv6', data, layout);

    //==============distancia relativa / tempo==============
    var distRobo = {
        x: tempFinal,
        y: vetDistancia,
        mode: 'lines+markers',
